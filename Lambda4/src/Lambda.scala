object Calculus {
  ...
  case class Environment(map: Map[Variable, Reference]) // extends Value 
  {
    ...
    def +(kvp: (Variable, Reference)): Environment = {
      Environment(map + kvp)
    }
  ...
  case class Apply(f: Expression, e: Expression) extends Expression {
    ...
    def evaluate(environment: Environment): Value = {
      println("->Apply: " + f,  e)
      val VLambda(Lambda(x, e1), environment1) = f.evaluate(environment)
      val v = Reference(CellClosure(e, environment.restrict(e.freeVariables))) // garbage collection
      val r = e1.evaluate(environment1 + (x -> v))
      println("<-Apply: " + r)
      r
    }
  }
