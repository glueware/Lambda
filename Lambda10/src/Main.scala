...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    def run[S](state: S, e: Expression[S, Any]) { println(e(state, Map.empty)) }
...
case class Query(param: Expression[Any, Int]) extends Expression[State, Int] {
  def apply(state: State, environment: Environment): Value[Int] = {
    Value(state.value(param(state, Map.empty).get))
  }
}
